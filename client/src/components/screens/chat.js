...
// import sendbutton from '../../img/home/sendbutton.png'
...
const Chat = () => {
    ...
    useEffect(()=>{
        async function fetchData(){
            await fetch(`/chat/conversation/${state ? state._id : "loading..."}`, {
                headers:{
    ...
    useEffect(()=>{
        console.log(currentChat);
        async function fetchData(){
            await fetch(`/chat/messages/${currentChat?._id}`, {
                headers:{
                    "Authorization":"Bearer " + localStorage.getItem("jwt")
                }
            }).then(res=> res.json())
                        ...
                        <>
                            ...
                            <div key={20} className="chatBoxBottom">
                                <textarea key={21}
                                    ...
                                    value= {newMessage}
                                ></textarea>
                                <button key={22} className='chatSubmitButton' onClick={handleSubmit}></button>
                            </div>
                        </>
